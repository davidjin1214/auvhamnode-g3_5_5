...
### 6.2 两份本地副本的分工（2026-08-20 起）

仓库有两份本地副本，职责不同。**工作只在工作副本里做，OneDrive 那份自 2026-08-20 起不再主动更新。**

| | 路径 | 职责 |
|---|---|---|
| **工作副本** | `D:\Codes\g3_5_5`（Windows 本机，非 OneDrive；与 `D:\Codes\rl_v2` 同一约定） | 论文写作与 LaTeX 编译、图脚本、代码改动、全部 git 操作。约 118 MB（其中 `.git` 92 MB） |
| **只读备份 + 数据来源** | `C:\Users\jinxiang\OneDrive\我的\Code\auv_se3node\g3_5_5` | 备份，并存放重实验产物：`checkpoints/`（14 GB）、`data/`（795 MB）、`analysis/oc_data_catalog/`（1.45 GB）。跨机同步、按需下载 |

工作副本的 remote：`origin` = GitHub（**唯一推送去向**），`onedrive` = 上表的 OneDrive 路径（保留只为按需 `git pull onedrive main` 或取历史，不再 push）。因此 OneDrive 副本的 git 状态会持续落后，**不要据它判断「最近的改动」**——判断以工作副本与 `origin` 为准。
...
- 代码与文档改动一律在工作副本提交并推到 `origin`；不再回推 `onedrive`。
