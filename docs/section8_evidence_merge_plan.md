...
前置事实（已在前序梳理中核验）：
- A 区 `noisy_train` 协议 = `iid_noisy_ic`（profile=nominal_train, ref=remus100_dr），**与 B 区 iid 噪声线同机制**；评估口径（60s / `final_position_error` median / `overall` / PRBS+CHIRP+OU×30 traj / 四 profile）一致。
- 交叉验证：A 区与 B 区在**非异常种子**上同尺度吻合（如 phnode_full clean 非漂移种子 A=0.61 vs B=0.68；no_mass_prior A=1.44 vs B=1.30）。
- 因此在 **标注来源 + 同镜像算倍数** 的前提下，A 区可信结果可与 B 区合并。
...
- 两基线相互验证（0.61 vs 0.68，差 ~10%），但倍数仍各自用本镜像基线，并在脚注注明。

## A.5 脚注 / 披露要求（写作时必须落实）

1. 每张合并表对**每一行标注来源镜像**（B 区 g3_5_7 / A 区 g3_5_5）。
2. 倍数表注明"同镜像基线"及其值。
3. 交叉验证一致性作为合并依据写入脚注或附录（附 phnode_full / no_mass_prior 的 A↔B 逐种子对照）。
...
| `se3_accel_blackbox`（保 SE(3)，黑箱加速度） | 2.46（B 区, N=5；尾部由 seed42 主导, P95≈9） | 5.46（A 区, N=3） | 保几何 → 有限但方差/尾部较大 |
...
| 耦合阻尼（D 改对角） | `ablate_diag_damping` | 4.17 | （待补） | **6.8×** | A 区 | 6 |
| 执行机构条件化（仅 actuator 状态） | `ablate_bu_only` | 27.8 | （待补） | **45.6×** | A 区 | 6 |

† `no_lift` N=4：seed43 为**真实可复现脆弱**（44.4m, nbad=276，多次重训复现，与环境无关），单列披露，不计入稳定簇中心趋势。见表 5。

**重要含义（需在写作中处理）**：合并后阶梯为
`bu_only(46×) ≫ diag_damping(6.8×) ≈ qforce 能量核心(5.5×) > no_mass_prior(1.9×) > no_lift(1.2×)`。
即 **耦合阻尼的退化幅度与能量核心相当、甚至略大**。当前 §8"能量核心是最主导精度先验"的表述需细化为：
- 执行机构条件化是**结构必需**（移除即崩溃）；
...
> A 区 `diag_damping` 6 种子中 5 个聚于 4–6m、仅 seed44=0.69 偏低，呈"多数差+个别好"，与环境漂移"多数好+个别坏"相反，支持 ~4m 为真实效应而非漂移。若需同镜像纯净数字，可在 B 区补跑 `diag_damping` clean×5（唯一值得补的定量单元）。
