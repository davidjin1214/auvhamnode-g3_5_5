...
**为什么在 `nu_r` 上加噪等价于在 `nu_total` 上加噪？**

DVL 底部跟踪测量的是体系坐标下的对地速度 `v_total`（不是对水速度 `v_r`），
因此 DVL 噪声 `ε_dvl` 在物理上作用于 `nu_total`。等价性推导：

```
nu_r_meas = (nu_total_true + ε_dvl) - R^T v_c^n
          = nu_r_true + ε_dvl
```

由于 `nu_r = nu_total - R^T v_c^n` 是线性变换，加性高斯噪声通过线性变换后仍为
加性高斯，分布不变。因此在 `nu_r` 上加等幅高斯噪声与在 `nu_total` 上加噪数学等价。

对于角速度分量，陀螺测量的是绝对角速度（`omega_total = omega_r`，与海流无关），
在 `nu_r` 或 `nu_total` 上加噪完全相同。

> **二阶效应注：** 若姿态同时有误差（R 被扰动），`R^T v_c^n` 的计算也会有误差。
> 该交叉项量级约为 `δR × v_c ≈ 0.017 × 0.2 ≈ 0.003 m/s`（nominal_eval 条件下），
> 与 DVL 噪声同量级。当前方案独立采样姿态和速度噪声，忽略了此交叉项，
> 作为初始条件扰动测试这一简化是可接受的。

### 4.3 各通道独立采样

当前实现对 `R`、`nu_r`、`u_act`、`v_c^n`、`depth_ref` 各通道独立采样高斯噪声。

在真实 EKF 后验中，这些状态分量的估计误差存在交叉相关（姿态误差通过旋转矩阵影响
速度估计，速度误差通过航位推算反馈到位置估计等）。但相关系数高度依赖 EKF 调参、
滤波器收敛状态和传感器可用性，是场景依赖的。

独立采样等价于对误差协方差矩阵做对角化假设，这会使某些不太可能的组合
（如大姿态误差+小速度误差）被覆盖到。对鲁棒性测试和训练正则化而言，
这是一个**更保守的简化**——覆盖了更广的初始条件不确定性空间。

### 4.4 block-relative 位置不作为独立噪声通道
...
**物理依据：** 海流估计不确定性的来源包括：
- DVL 底部跟踪与水体跟踪差值法（`v_c ≈ v_BT - v_WT`）：精度为两者 RSS ≈ √2 × 0.005 ≈ 0.007 m/s
- ADCP 海流剖面估计：典型精度 0.5–2 cm/s
- 海流时空变化导致"记忆"值与实际值的偏差

`nominal_eval` 的 0.012 m/s 大致对应 DVL 差值法精度加上时空变化余量。
垂向取水平一半，反映海流场在垂直方向上变化通常较慢、DVL 垂直波束精度更高。
