...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/eval_all_models_noise.sh [options]

Run rollout evaluation, sweep summary, and experiment report for a noise
training sweep. If no suite is provided, the script picks the latest
noise-tagged sweep under checkpoints/.
...
Examples:
  bash scripts/eval_all_models_noise.sh
  bash scripts/eval_all_models_noise.sh --suite-name sweep_oc_all_noise_l2_...
  bash scripts/eval_all_models_noise.sh --suite-dir ./checkpoints/my_suite --device cuda:0
EOF
}
...
if [[ -z "${SUITE_DIR}" ]]; then
  if [[ -n "${SUITE_NAME}" ]]; then
    ...
    SUITE_DIR="$(latest_noise_suite)"
  fi
fi

if [[ -z "${SUITE_DIR}" || ! -d "${SUITE_DIR}" ]]; then
  echo "Noise sweep directory not found." >&2
...
echo
echo "Noise sweep evaluation complete."
