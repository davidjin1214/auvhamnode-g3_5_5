...
当前正式主稿已建立并完成第 1--4 节的主要正文返工、复核修订和正式表达润色。下一步只做三件事：

1. 重构第 5 节“结构化连续时间动力学模型”，优先补强相对动量动力学、非保守广义力分支和执行器通道之间的论证衔接。
2. 重构第 6 节“结构化模型的能量性质与功率关系”，给出存储函数、耗散/零功率/广义力功率关系、静水功率平衡命题和适用范围。
