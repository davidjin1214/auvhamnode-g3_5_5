...
| Fig. 4 | 从 Fossen/pH 到 AUVHamNODE 的结构桥梁图 | Fossen 能量结构、REMUS100 工程复杂性、PH-NODE 事前结构约束 | 支撑新增第 4 节论证 | 待制作 |
...
1. **从 Fossen 能量结构到结构保持学习模型**：先写新增第 4 节，建立 Fossen/pH 能量结构、REMUS100 工程复杂性和 PH-NODE 事前结构约束之间的桥梁。
2. **结构化连续时间动力学模型**：依次补强 \(\mathrm{SE}(3)\) 运动学、六自由度机械子系统、非保守力分解、执行器和外源通道、相对动量动力学。
3. **结构化模型的能量性质与功率关系**：给出存储函数、耗散/零功率/广义力功率关系、静水功率平衡命题、海流和增强状态适用范围。不要把正式标题写成“机械核心能量平衡与理论声称边界”。
4. **训练目标、基线体系与验证协议**：把理论对象映射到结构保持参数化，写清控制块损失、长期递推、基线链条、结构消融和 evidence status 口径。
5. **实验结果与结构证据分析**：按 evidence status 过滤后再填入结果；结果章应单独呈现 current evidence，而不是混入协议节。
6. **相关建模基础**：当前主稿第 2 节已完成 v1，并已加入文献角色表及一般受控流到本文增强状态外源变量定义的过渡；后续仅随方法章符号、功率边界和新增引用做一致性修订，不扩展成泛泛综述。
7. **讨论**：主动写非闭合 pH、非严格李群积分、非唯一水动力辨识、仿真证据和真实海试泛化边界。
8. **研究问题、摘要和本章小结**：最后修订，使其准确概括已经完成的正文。第 1 节应建立问题张力、研究缺口和方法切入点，不应把第 3--7 节的概念压缩为一串术语。
...
5. 速度变量关系图已完成并接入第 3 节；下一步生成“六自由度机械子系统能量/功率结构图”和“Fossen/pH 到 AUVHamNODE 的结构桥梁图”，这两张图仍应早于实验结果图完成。
...
当前正式主稿已建立并完成第 1--3 节的主要正文返工、复核修订和正式表达润色。下一步只做三件事：

1. 在 `paper/drafts/auvhamnode_thesis_chapter_zh.tex` 中新增或重构第 4 节“从 Fossen 能量结构到结构保持学习模型”，优先把 `fossen_to_ph_node_auv_report.md` 的主线转化为正式正文。
2. 重构第 5--6 节：第 5 节写主方法“结构化连续时间动力学模型”，第 6 节写“结构化模型的能量性质与功率关系”。
3. 运行或编写结果导出脚本，将 current evidence 的 rollout 主表导出为论文表格底稿。

前两件事推进理论和方法正文，第三件事锁定证据。不要再继续扩展指南，除非正式写作中发现具体缺口。
