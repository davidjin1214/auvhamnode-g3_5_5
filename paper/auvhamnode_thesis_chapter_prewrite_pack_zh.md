...
1. **从 Fossen 能量结构到结构保持学习模型**：该节已完成 v2 修订，建立 Fossen 型能量--功率角色、工程化 AUV 建模差异、端口哈密顿适用边界和事前结构约束之间的桥梁。
...
5. 速度变量关系图已完成并接入第 3 节；下一步生成“六自由度机械子系统能量/功率结构图”和“从 Fossen 型功率结构到 AUVHamNODE 的结构桥梁图”，这两张图仍应早于实验结果图完成。
...
当前正式主稿已完成第 1--5 节的主要正文返工、复核修订和正式表达润色，并已切入 10 节结构：第 6 节为“结构化模型的能量性质与功率关系”，第 7 节为“训练目标、基线体系与验证协议”，第 8 节为“实验结果与结构证据分析”。下一步只做三件事：

1. 扩写第 6 节“结构化模型的能量性质与功率关系”，给出存储函数、耗散/零功率/广义力功率关系、静水功率平衡命题和适用范围。
2. 扩写第 7 节“训练目标、基线体系与验证协议”，补强结构保持参数化、控制块损失、长期递推、基线体系、结构消融和 evidence status 口径。
3. 运行或编写结果导出脚本，将 current evidence 的 rollout 主表导出为论文表格底稿，再填写第 8 节结果分析。

前两件事推进理论、方法和验证协议正文，第三件事锁定证据。不要再继续扩展指南，除非正式写作中发现具体缺口。
