...
5. **相关建模基础**：当前主稿第 2 节已完成 v1，并已加入文献角色表及一般受控流到本文增强状态外源变量定义的过渡；后续仅随方法章符号、功率边界和新增引用做一致性修订，不扩展成泛泛综述。
...
当前正式主稿已建立并完成第 1--3 节的主要正文返工、复核修订和正式表达润色。下一步只做两件事：

1. 继续扩写 `paper/drafts/auvhamnode_thesis_chapter_zh.tex` 第 4 节“结构化连续时间动力学模型”，优先补强相对动量动力学、非保守广义力分支和执行器通道之间的论证衔接，并保持第 3 节中分段常值外源变量和执行器一阶滞后的状态表示约定。
