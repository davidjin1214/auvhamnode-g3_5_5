...
| [drafts/auvhamnode_thesis_chapter_zh.tex](drafts/auvhamnode_thesis_chapter_zh.tex) | 当前正式重写主稿骨架 | 当前文件仍保留 8 节草稿骨架；后续应按本文档第 5 节记录的 10 节扩展结构重构第 4 节以后 | 后续正文扩写的唯一主稿入口 |
...
2026-05-22 后续章节结构复核后的补充规则：第 1--3 节基本保留，不再继续扩写为新的综述或方法清单；第 4 节以后应从旧的 8 节压缩方法章调整为 10 节扩展结构。新增桥梁节“从 Fossen 能量结构到结构保持学习模型”，用于承接 Fossen/pH 功率结构、REMUS100 工程复杂性和 PH-NODE 的事前结构约束思想；原“能量平衡与理论边界”应改名为“结构化模型的能量性质与功率关系”，作为主方法的结构性质分析，而不是与主方法并列的新贡献。“机械核心”可在正文中经过定义后作为分析对象或内部简称使用，但不作为正式一级标题中心词；“理论声称边界”不作为正式标题，应转化为“适用条件”“功率关系”“能量性质”等学术表达。后续实验结果应在 current evidence 主表导出后单独成节，不再只附着在“训练目标与验证协议”中。
...
| 正文草稿 | in_progress | `drafts/auvhamnode_thesis_chapter_zh.tex` 当前仍为 8 节草稿骨架 | 按第 5 节记录的 10 节结构重构第 4 节以后 |
...
| Fossen-to-structure 桥梁节 | ready | `fossen_to_ph_node_auv_report.md` 已有完整素材 | 在主稿第 4 节新增“从 Fossen 能量结构到结构保持学习模型” |
| 方法正文 | in_progress | 当前主稿第 4--5 节已有 SE(3)、能量、非保守力结构和能量命题 | 重构为第 5 节“结构化连续时间动力学模型”，补强物理动机和公式衔接 |
| 能量性质正文 | ready | 当前主稿第 5 节已有命题、证明和边界说明 | 改为第 6 节“结构化模型的能量性质与功率关系”，避免用“理论声称边界”作正式标题 |
| 评估设置正文 | ready | 当前主稿第 6 节已有训练目标、长期递推和结构消融设置 | 重构为第 7 节“训练目标、基线体系与验证协议”，结果另起第 8 节 |
...
2026-05-22 复核后，正式正文建议采用以下 10 节结构。当前 TeX 主稿仍是 8 节草稿骨架，后续应优先重构第 4 节以后；第 1--3 节只随符号和引用做局部一致性修订。
...
4. **从 Fossen 能量结构到结构保持学习模型**
   ...
   - REMUS100 风格工程模型中的连续车辆动力学核心与工程复杂性
   - 传统 Fossen/pH 建模在海流、执行器和经验水动力上的局限
...
| P3a | 新增“从 Fossen 能量结构到结构保持学习模型” | 正文第 4 节完整正文 v1 | ready |
| P3b | 重构“结构化连续时间动力学模型” | 正文第 5 节完整正文 v1 | in_progress |
| P4 | 扩写“结构化模型的能量性质与功率关系” | 正文第 6 节命题、证明和适用范围完整版本 | ready |
| P5 | 扩写“训练目标、基线体系与验证协议” | 正文第 7 节参数化、损失、rollout、基线和消融完整版本 | ready |
...
第 1--3 节返工、复核和正式表达润色已完成，可以恢复 P3。P1e 已将第 1 节改写为 introduction v4：主线不再是后续技术点的提前罗列，而是从 AUV 运动建模和长期状态预测任务出发，依次说明传统参数化模型、数据驱动模型和结构化神经动力学的作用与缺口，最后引出本文的 AUVHamNODE。P1f 已将第 2 节扩写为长论文级“相关建模基础”v1：内容覆盖 AUV 六自由度运动与能量--功率关系、水动力参数获取与模型校准边界、受控时序预测与 Neural ODE、科学机器学习、哈密顿和端口哈密顿结构化建模基础，并补充文献角色归纳表。P2/P2r/P2s 已将第 3 节扩写、复核并润色为数据态、模型态、输出态和分段常值外源变量的状态表示约定，同时把第 1 节的文献缺口表述收束为任务特定的研究需求，而非绝对化的“既有研究缺失”判断。
...
12. 将第 1 节关于既有数据驱动方法的不足改为有条件、有任务边界的表述，避免在未做系统综述时把研究缺口写成绝对缺失。
13. 删除或改写前 3 节中残留的章节安排式元话语，把它们转化为状态定义、论证关系或模型边界。

P3 的下一步改为：

1. 先写新增第 4 节“从 Fossen 能量结构到结构保持学习模型”，吸收 `fossen_to_ph_node_auv_report.md` 中 Fossen 能量结构、REMUS100 工程复杂性、PH-NODE 事前结构约束三段论证。
2. 再重构第 5 节“结构化连续时间动力学模型”，优先补强相对动量动力学、非保守广义力分支和执行器通道之间的论证衔接。
3. 第 6 节使用“结构化模型的能量性质与功率关系”作为正式标题；正文可定义六自由度机械子系统或机械部分，但不要把“机械核心能量平衡与理论声称边界”作为标题。
4. 检查第 5--6 节是否延续第 2--3 节的功率和外源边界：只分析六自由度机械子系统的端口哈密顿风格功率关系，不把完整增强系统写成闭合标准端口哈密顿系统；控制命令、海流和深度上下文应保持分段常值外源变量定义。
5. 确认第 5--6 节中所有 \(R\)、\(J\)、\(D\)、\(\mathcal R\)、\(J_\theta\)、\(D_\theta\) 的符号角色与第 2 节一致。
6. 第 4--6 节完成后，回读第 2--3 节，只做必要的符号和边界一致性修订；不要再把前 3 节扩展成新的综述或方法清单。
