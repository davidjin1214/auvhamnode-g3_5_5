...
| 第 1--2 节文献支撑返工 | done | 第 1 节已重写为以 AUV 运动建模为中心、长期状态预测为任务场景的 introduction v4；第 2 节已扩写为长论文级“相关建模基础”v1，并补强 Fossen 功率边界、受控时序预测、Neural ODE、科学机器学习、哈密顿/端口哈密顿神经模型和文献角色归纳表 | 后续仅随方法章符号和引用调整做一致性修订；终稿前对 2024--2026 新近文献做 citation audit |
...
第 1--3 节返工和复核修订已完成，可以恢复 P3。P1e 已将第 1 节改写为 introduction v4：主线不再是后续技术点的提前罗列，而是从 AUV 运动建模和长期状态预测任务出发，依次说明传统参数化模型、数据驱动模型和结构化神经动力学的作用与缺口，最后引出本文的 AUVHamNODE。P1f 已将第 2 节扩写为长论文级“相关建模基础”v1：内容覆盖 AUV 六自由度运动与能量--功率关系、水动力参数获取与模型校准边界、受控时序预测与 Neural ODE、科学机器学习、哈密顿和端口哈密顿结构化建模基础，并补充文献角色归纳表。P2/P2r 已将第 3 节扩写并复核为数据态、模型态、输出态和分段常值外源变量的状态表示约定。
...
8. 在第 2 节中明确 Fossen 型能量--功率关系的机械子系统边界，避免把执行器、海流外源和神经广义力增强系统写成闭合严格端口哈密顿系统。
9. 将端口哈密顿模板中的耗散矩阵记为 \(\mathcal R(z)\)，避免与旋转矩阵 \(R\) 混淆。
10. 将第 2 节的一般受控流表述与第 3 节的本文状态表示分层处理：第 2 节保留理论宽度，第 3 节明确分段常值外源变量和执行器一阶滞后。
11. 在第 3 节补入仿真基准状态可观测性边界，避免把仿真中可保存的 \(u_a\)、\(v_c^n\)、\(z_{\mathrm{ref}}\) 自动外推为真实海试条件下的直接观测量。

P3 的下一步：

1. 扩写第 4 节“结构化连续时间动力学模型”，优先补强相对动量动力学、非保守广义力分支和执行器通道之间的论证衔接。
2. 检查第 4--5 节是否延续第 2--3 节的功率和外源边界：只把机械核心写成端口哈密顿风格能量平衡，不把完整增强系统写成闭合标准端口哈密顿系统；控制命令、海流和深度上下文应保持分段常值外源变量定义。
