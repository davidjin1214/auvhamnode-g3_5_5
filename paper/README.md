...
| 第 1--2 节文献支撑返工 | in_progress | 当前第 1 节 introduction 依据不足，第 2 节 preliminaries 过薄 | 先查证并重写第 1--2 节，再继续模型章 P3 |
...
1. **研究问题与方法概述**
   - AUV 长期预测难点：六自由度耦合、海流、执行器和姿态几何
   - 参数化物理建模、水动力系数获取/校准、黑箱学习和结构化科学机器学习的关系
   - 方法定位、简称和理论声称边界
...
2. **相关建模基础**
   ...
   - Hamiltonian、port-Hamiltonian 与 AUV 功率结构
...
| P1e | 重写第 1 节 introduction 论证 | 文献支撑的“研究问题与方法概述”v1 | in_progress |
...
当前暂停 P3，先回头重写第 1--2 节。原因是第 1 节目前把“理论分析、CFD、系统辨识”和“深度学习”写成了简化分类，文献支撑和层级关系不足；第 2 节目前更接近短论文 preliminaries，不能支撑博士论文章节的理论铺垫。

P1e 需要完成以下内容：

1. 查证并整理 AUV 长期预测的核心困难：六自由度耦合、附加质量、非线性阻尼、恢复力、执行器、海流和姿态几何。
2. 将“传统方法”改写为更准确的层级：Fossen 型参数化六自由度动力学是建模主线，CFD/EFD/拖曳或 PMM 实验/自由航行试验/系统辨识主要服务于水动力系数获取、模型校准和验证。
3. 将“深度学习”改写为从黑箱受控时序预测到结构化科学机器学习的递进，区分离散状态转移、Neural ODE、PINN/UDE、Hamiltonian/pH 结构学习。
4. 为第 1 节补入可核验文献支撑，避免使用未经查证的口头分类。
5. 保持中心定位：本文贡献是结构化连续时间假设空间，不是完整严格端口哈密顿化工程模拟器。
...
4. 扩写受控神经时序预测、Neural ODE、科学机器学习、Hamiltonian 和 port-Hamiltonian 神经模型的关系。
