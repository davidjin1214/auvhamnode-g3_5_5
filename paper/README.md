...
| P10 | 终稿级数值、术语和图表复核 | 第 8 节表格核对与追踪清单更新 | done：§8 主表、扰动表、轻量协议表和内部诊断表已与 `analysis/section8_current_evidence/` 对齐；本轮不新增结果图 |
...
2026-05-31 已完成第 1.4 节终稿级桥梁重写。下一轮优先处理第 1.5 节：保持“模型定义节”定位，清理任何第 8 节结果预告或性质证明口吻，并检查 \(\xi\)、\(c_\tau\)、\(p_r\)、\(\tau_\theta\)、\(J_\theta\)、\(D_\theta\) 与第 1.4 桥接表及第 1.6 能量性质定义之间的接口一致性。
...
4. **图表补强判断**：当前主要结论已由速度变量关系图和第 8 节表格支撑，本轮不新增结果图。若后续扩图，优先考虑概念性“能量/功率结构图”或“模型结构阶梯图”；在未形成统一时间序列导出前，不建议新增 rollout 误差增长图。
5. **编译验证**：本轮未修改 TeX 正文；若后续改正文或接入新图，仍需重新运行 LaTeX 编译，检查 undefined reference/citation、表格溢出和 PDF 版面。
