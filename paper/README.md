...
| 正文草稿 | in_progress | `drafts/auvhamnode_thesis_chapter_zh.tex` 第 1--5 节已完成阶段性正文，10 节结构已切入主稿，`drafts/auvhamnode_thesis_chapter_zh.pdf` 已按最新 TeX 编译更新；2026-05-23 已对第 1--5 节做审稿式复核修订（§2/§4 去重、补对标文献、§5–§6 记号与机械核心定义一致化、§1 增补贡献清单），PDF 已重新编译 | 继续扩写第 6--7 节，并在 current evidence 表导出后填写第 8 节 |
| 第 1--2 节文献支撑返工 | done | 第 1 节已重写为以 AUV 运动建模为中心、长期状态预测为任务场景的 introduction v4；第 2 节已扩写为长论文级“相关建模基础”v1，并补强 Fossen 功率边界、受控时序预测、Neural ODE、科学机器学习、哈密顿/端口哈密顿神经模型和文献角色归纳表；2026-05-23 复核中于第 2.5 节增补 SE(3) (p)H-NODE 与拉格朗日神经网络对标差异段及李群几何积分文献 | 终稿前核实新增对标文献著录（见“对标文献著录核实”项）并完成 2024--2026 citation audit |
| 对标文献著录核实 | watch | 已加入 `duong2021se3hamnode`、`duong2024liegroupphnode`、Lagrangian NN/DeLaN/Finzi、李群几何积分等共 7 条引用 | 终稿前核实 RSS 2021 / IEEE T-RO 2024 的卷期、DOI 与作者列表，补全可确认的 DOI |
...
| 能量性质正文 | in_progress | 当前主稿第 6 节已切换为“结构化模型的能量性质与功率关系”，并保留已有命题、证明和边界说明 | 扩写为第 5 节主方法的结构性质分析，补强适用条件、功率关系和增强状态边界 |
| 评估设置正文 | in_progress | 当前主稿第 7 节已切换为“训练目标、基线体系与验证协议”，并保留训练目标、长期递推和结构消融设置 | 补强基线体系、证据口径和与第 8 节结果分析的边界 |
...
2026-05-23 复核后，正式正文建议采用以下 10 节结构。当前 TeX 主稿第 1--5 节已按该结构完成阶段性正文；第 6--7 节已切换为新标题并保留待扩写正文；第 8 节结果分析入口已建立。第 1--5 节后续只随符号、引用和后文一致性做局部修订。
...
| P4 | 扩写“结构化模型的能量性质与功率关系” | 正文第 6 节命题、证明和适用范围完整版本 | in_progress |
| P5 | 扩写“训练目标、基线体系与验证协议” | 正文第 7 节参数化、损失、rollout、基线和消融完整版本 | in_progress |
...
第 1--5 节已形成连续论证链：第 1 节建立 AUV 运动建模和长期状态预测任务，第 2 节给出相关建模基础，第 3 节固定数据态--模型态和海流速度契约，第 4 节完成 Fossen 能量结构到结构保持学习模型的桥梁，第 5 节给出结构化连续时间动力学模型 v1。当前主稿也已切入 10 节结构：第 6 节为“结构化模型的能量性质与功率关系”，第 7 节为“训练目标、基线体系与验证协议”，第 8 节为“实验结果与结构证据分析”。

当前应转入 P4。第 6 节需要在保留已有命题、证明和边界说明的基础上，扩写为第 5 节主方法的结构性质分析：先定义六自由度机械子系统和存储函数，再说明耗散、零功率耦合和广义力功率配对，随后给出静水条件下的功率平衡命题和证明，最后说明海流、执行器、增强状态和普通 ODE 数值积分的适用范围。

P4 完成后再推进 P5：第 7 节应补强结构保持参数化、控制块训练目标、长期递推、基线体系、结构消融和 evidence status 口径。第 8 节必须等 current evidence 主表导出后再填写具体结果和排序，不提前写性能结论。
