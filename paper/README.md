...
| 正文草稿 | in_progress | `drafts/auvhamnode_thesis_chapter_zh.tex` 已新增第 4 节桥梁正文，`drafts/auvhamnode_thesis_chapter_zh.pdf` 已按最新 TeX 编译更新 | 按第 5 节记录的 10 节结构继续重构第 5 节以后 |
...
| Fossen-to-structure 桥梁节 | done | 当前主稿第 4 节“从 Fossen 能量结构到结构保持学习模型”v1，最新 PDF 已编译 | 后续只随第 5--6 节符号和功率边界做一致性修订 |
| 方法正文 | in_progress | 当前主稿第 5 节已有 SE(3)、能量、非保守力结构和能量命题 | 重构为第 5 节“结构化连续时间动力学模型”，补强相对动量动力学、非保守广义力分支和执行器通道之间的论证衔接 |
| 能量性质正文 | ready | 当前主稿第 6 节已有命题、证明和边界说明 | 改为第 6 节“结构化模型的能量性质与功率关系”，避免用“理论声称边界”作正式标题 |
| 评估设置正文 | ready | 当前主稿第 7 节已有训练目标、长期递推和结构消融设置 | 重构为第 7 节“训练目标、基线体系与验证协议”，结果另起第 8 节 |
...
| P3a | 新增“从 Fossen 能量结构到结构保持学习模型” | 正文第 4 节完整正文 v1 + 最新 PDF 编译 | done |
...
2. 第 6 节使用“结构化模型的能量性质与功率关系”作为正式标题；正文可定义六自由度机械子系统或机械部分，但不要把“机械核心能量平衡与理论声称边界”作为标题。
3. 检查第 5--6 节是否延续第 2--4 节的功率和外源边界：只分析六自由度机械子系统的端口哈密顿风格功率关系，不把完整增强系统写成闭合标准端口哈密顿系统；控制命令、海流和深度上下文应保持分段常值外源变量定义。
4. 确认第 5--6 节中所有 \(R\)、\(J\)、\(D\)、\(\mathcal R\)、\(J_\theta\)、\(D_\theta\) 的符号角色与第 2 节一致。
5. 第 5--6 节完成后，回读第 2--4 节，只做必要的符号和边界一致性修订；不要再把前 3 节扩展成新的综述或方法清单。
