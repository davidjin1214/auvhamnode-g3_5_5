#!/usr/bin/env python3
"""Draw the Fossen-role -> structure-preserving-component mapping for §1.4.
...
OUT_BASE = OUT_DIR / "fossen_role_mapping"
...
def draw_rows(ax) -> None:
    ...
    left_box(ax, LY_RESTORING, "Restoring", r"$g(\eta)$")
...
def draw_anchors_and_footnote(ax) -> None:
    ...
    add_text(ax, 0.035, 0.262, r"$M\dot\nu+C(\nu)\nu+D(\nu)\nu+g(\eta)=\tau$", size=MATH_SIZE - 0.6, color=PALETTE["ink"], ha="left")
